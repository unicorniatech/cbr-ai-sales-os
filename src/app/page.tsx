      ...
      <div className="sticky top-0 h-[100svh] overflow-hidden bg-[#030a16] md:h-screen">
        ...
        <div className="absolute inset-x-0 top-0 z-10 flex items-center justify-between gap-4 px-5 py-5 text-sm text-white/80 sm:px-10">
          ...
          <nav className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-white/72">
            ...
            <a
              href="#contacto"
              className="inline-flex items-center gap-2 border border-white/18 bg-white/[0.07] px-3 py-2 text-white backdrop-blur transition hover:border-[#d7b56d]/70 hover:text-[#f3d99a]"
            >
              Contacto
...
function ImmersiveWalkthroughSection() {
  ...
  useEffect(() => {
    ...
    const initScene = async () => {
      ...
      scene.fog = new THREE.Fog("#071321", 9, 18);
      ...
      const ambientLight = new THREE.AmbientLight("#f5deb3", 1.15);
      const keyLight = new THREE.DirectionalLight("#f3d99a", 2.2);
      keyLight.position.set(-4, 8, 5);
      scene.add(ambientLight, keyLight);

      const ground = new THREE.Mesh(
        new THREE.PlaneGeometry(12, 7.4, 36, 22),
      ...
      const grid = new THREE.GridHelper(11, 12, "#d8b86f", "#ffffff");
      ...
      const pathVectors = walkthroughPathPoints.map(([x, y, z]) => new THREE.Vector3(x, y + 0.08, z));
      const pathGeometry = new THREE.BufferGeometry().setFromPoints(pathVectors);
      const pathLine = new THREE.Line(
        pathGeometry,
        new THREE.LineBasicMaterial({ color: "#f3d99a", transparent: true, opacity: 0.88 }),
      );
      scene.add(pathLine);
      ...
      const pointGeometry = new THREE.SphereGeometry(0.11, 24, 16);
      const pointMeshes = pathVectors.map((position, index) => {
        const mesh = new THREE.Mesh(pointGeometry, inactiveMaterial.clone());
      ...
      const onPointerDown = (event: PointerEvent) => {
        ...
        const [hit] = raycaster.intersectObjects(pointMeshes);
        if (hit?.object?.userData && typeof hit.object.userData.index === "number") {
          selectFrame(hit.object.userData.index);
        }
      };
      ...
      const animate = (time: number) => {
        const activeIndex = activeFrameRef.current;
        const target = pathVectors[activeIndex] ?? pathVectors[0];
        const orbit = Math.sin(time * 0.00035) * 0.45;

        camera.position.x += (target.x * 0.28 + orbit - camera.position.x) * 0.035;
        camera.position.z += (target.z + 8.2 - camera.position.z) * 0.035;
        camera.lookAt(target.x * 0.36, 0.05, target.z * 0.42);

        pointMeshes.forEach((mesh, index) => {
          const isActive = index === activeIndex;
          mesh.material = isActive ? pointMaterial : inactiveMaterial;
          const scale = isActive ? 1.85 + Math.sin(time * 0.006) * 0.12 : 1;
  ...
  return (
    <section className="relative overflow-hidden bg-[#030a16] py-24 sm:py-32">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_24%,rgba(216,184,111,0.16),transparent_34%),linear-gradient(180deg,#030a16_0%,#071321_58%,#030a16_100%)]" />
        ...
        <Reveal>
          <div className="mb-12 grid gap-8 lg:grid-cols-[0.86fr_1.14fr] lg:items-end">
            <div>
              ...
              <h2 className="text-balance text-4xl font-semibold leading-tight text-white sm:text-6xl">
                Una ruta visual para sentir el terreno antes de visitarlo.
              </h2>
            </div>
            <p className="max-w-2xl text-lg leading-8 text-white/62">
              Los puntos siguen el orden real de las fotos. Puedes avanzar por la ruta o tocar un punto del plano para saltar a esa vista.
            </p>
          </div>
        </Reveal>

        <Reveal delay={0.1}>
          <div className="grid overflow-hidden border border-white/10 bg-white/[0.035] lg:grid-cols-[1fr_0.86fr]">
            <div className="relative min-h-[430px] border-b border-white/10 bg-[#071321] lg:min-h-[620px] lg:border-b-0 lg:border-r">
              <canvas ref={canvasRef} className="absolute inset-0 h-full w-full" aria-label="Plano 3D del recorrido" />
              <div className="pointer-events-none absolute left-4 top-4 border border-white/10 bg-[#030a16]/72 px-3 py-2 text-xs uppercase tracking-[0.18em] text-[#f3d99a] backdrop-blur">
                Punto {activeFrame + 1} de {walkthroughFrames.length}
              </div>
              <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-[linear-gradient(180deg,transparent,rgba(3,10,22,0.86))] p-5">
                <div className="h-1 overflow-hidden bg-white/15">
                  <div
                    className="h-full bg-[#d8b86f] transition-all duration-300"
                    style={{ width: `${((activeFrame + 1) / walkthroughFrames.length) * 100}%` }}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex min-h-[560px] flex-col">
              <div className="relative min-h-[390px] flex-1 overflow-hidden bg-black">
                <AnimatePresence mode="wait">
                  ...
                  >
                    <Image src={frame.src} alt={frame.label} fill sizes="(max-width: 1024px) 100vw, 560px" className="object-cover" />
                  </motion.div>
                </AnimatePresence>
                <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(3,10,22,0.04),rgba(3,10,22,0.72))]" />
                <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-7">
                  <p className="text-xs uppercase tracking-[0.24em] text-[#f3d99a]">{frame.label}</p>
                  <h3 className="mt-2 text-3xl font-semibold text-white sm:text-4xl">Vista del recorrido</h3>
                </div>
              </div>
              ...
              <div className="grid gap-4 border-t border-white/10 p-5">
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => selectFrame(activeFrame === 0 ? walkthroughFrames.length - 1 : activeFrame - 1)}
                    className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 border border-white/12 text-sm font-semibold uppercase tracking-[0.12em] text-white/72 transition hover:border-[#d8b86f]/70 hover:text-[#f3d99a]"
                  >
                    <ChevronLeft size={18} />
                    Atrás
                  </button>
                  <button
                    type="button"
                    onClick={() => selectFrame((activeFrame + 1) % walkthroughFrames.length)}
                    className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 bg-[#d8b86f] text-sm font-semibold uppercase tracking-[0.12em] text-[#07111f] transition hover:bg-[#f3d99a]"
                  >
                    Avanzar
                    <ChevronRight size={18} />
                  </button>
                </div>
                <div className="grid gap-2" style={{ gridTemplateColumns: "repeat(19, minmax(0, 1fr))" }}>
                  {walkthroughFrames.map((item, index) => (
                    <button
                      ...
                      className={`h-2 transition ${
                        activeFrame === index ? "bg-[#d8b86f]" : "bg-white/18 hover:bg-white/42"
                      }`}
